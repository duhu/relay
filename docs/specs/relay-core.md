...
| D11 | 输入源改为从显示器报上来的清单里按名字选，界面不显示编号；「现在显示的」那个用 `<optgroup>` 分组标题标出，不写进选项文字 | 编号（16/17/18）对用户没有意义，当初配笔记本时只能靠试。显示器自己知道它有几个口：AOC 答 `60(0F 10 11 12)`，四选一。名字用 MCCS 标准名，**不是机身上印的字**——哪个物理口是 "DisplayPort 1" 显示器不说，靠「现在显示的」补上。标记不写进选项文字，是因为收起来的 `<select>` 框宽度固定：`DisplayPort 2 · 当前` 在 136px 里放不下，而分组标题只在展开时出现 |
...
- 能力读取：`display/capabilities.rs` 按 DDC/CI 分片协议取回能力串——请求 `[0x83, 0xF3, offHi, offLo, chk]`（校验和与读输入源同规则，只用 `0x6E` 起头、不异或 `0x51`），应答 `buf[2] == 0xE3`、`buf[3..5]` 回显 offset、`buf[5..2+len]` 是正文，`len == 3`（正文为空）即读完，下一轮 offset = 已收到的字节数。三道闸：空片停、最多 64 轮、最多 2048 字节；回显 offset 对不上立刻放弃。`len` 限定在 `3..=35`（正文最多 32 字节），否则读缓冲区尾部的总线数据会被拼进串里。从 `vcp(...)` 段按「两位十六进制特征码 [+ 可选括号取值表]」逐个走，取 `60(...)` 的取值表（不是 `find("60(")`：别的特征码的取值表里也会出现 `60`）。配 MCCS 标准的编号→名字表（0x0F = DisplayPort 1、0x11 = HDMI 1 …），认不出的编号无名字。读不出来 = 空列表（不是错误），设置页退回手填编号。整个过程在一次 `spawn_blocking` 里、8 秒超时；实测这台 AOC 取完 338 字节要 24 轮、约 0.95 秒，只在设置页（打开窗口、添加显示器、扫描显示器）用，不进切换路径。不重试（与 0x60 读不同）：读不出来那一轮就退回手填编号，重问的唯一入口是设置页的「扫描显示器」。
...
- **导出 / 导入**（设置页「高级」）：导出的是**磁盘上正在生效的那份**（Rust 端整文件复制，不是窗口里那份未保存的编辑），导出到活配置文件自身会被拒绝。导入时整份文件照单全收，只有两处按本机改写（`Config::adopt`）：
  1. `this_host` 换成用户在向导里选的那一台；
  2. 每个 `is_trigger` 设备的 `leave_to` 换成「从本机离开时送去哪」——两台机器时就是另外那一台（不用问），三台及以上时是向导问出来的那一台（不给则拒绝导入）；非触发设备的 `leave_to` 一律置 `None`（文件里那个是上一台机器的事）。
  文件的 `schema_version` 大于当前版本、或改写后 `validate()` 不过，则整个导入失败，活配置一字节不动。
- **向导**：首次运行（没有配置文件、配置加载不出来、或 `this_host` 不在 `hosts` 里）时 app 自行打开设置窗口并进向导；之后也能从设置页「高级」的「重新运行向导」进。开头先分岔：
  - **第一台**（四步）：输入监控授权 → 扫描 HID++ 设备、各选一个触发/跟随（设备报出的通道号即本机是哪一台）→ 命名共用这套键鼠的机器 → 选共用显示器并为每台机器挑输入源（可跳过，之后在设置里补）。最后一步问要不要导出一份，供下一台导入。
  - **已有一台配好**：选那份导出的文件，按上一条的两条规则改写后写入。
  向导写配置走的是与设置页保存同一条 `save_config` 路径（校验 + 原子写），中途退出则什么都不写。
...
| 项 | 做法 |
|---|---|
| 首次运行 | **不写任何示例配置**：没有配置文件的机器让核心停在 `Unconfigured`，app 自行打开设置窗口进向导（§6），第一份配置由向导写 |
